...
from .twin import INFERRED, MEASURED, SIMULATED, Twin
...
def operator(twin: Twin, station: str) -> str:
    ...
    lines.append(f"  in-buffer  {MARK[twin.buffers[station].source]} {twin.buffers[station].value}/{cfg.buffer_before}")
    lines.append(f"  sensors    {cfg.sensors.name}")
...
def supervisor(twin: Twin) -> str:
    cfg = twin.cfg
    lines = [f"SUPERVISOR  line {cfg.name}   t={_hm(twin.t)}   out {twin.exited}   {LEGEND}"]
    lines.append(f"  {'stn':<5}{'zone':<7}{'state':<10}{'cycle':>8}{'buf':>6}  alert")
    for s in cfg.stations:
        b = twin.stations[s.id]
        cyc = b.cycle_s
        cyc_s = "   -   " if cyc.value is None else f"{MARK[cyc.source]}{cyc.value:5.1f}s"
        buf = twin.buffers[s.id]
        a = twin.active.get(s.id)
        alert = "" if a is None else f"○ blocks in ~{a.eta_s / 60:.0f}m ({a.confidence:.0%})"
        lines.append(f"  {s.id:<5}{s.zone:<7}{MARK[b.state.source]}{b.state.value:<9}"
                     f"{cyc_s:>8}{MARK[buf.source]}{buf.value:>2}/{s.buffer_before:<2} {alert}")
    return "\n".join(lines)


def manager(twin: Twin, scorecard: dict | None = None, coverage: dict[str, str] | None = None) -> str:
    cfg = twin.cfg
    ...
    if scorecard:
        ...
        for s in scorecard["scores"]:
            if s.lead_s is not None:
                outcome = f"{s.lead_s / 60:.1f} min lead"
