...
def score(provider: llm.Provider | None = None) -> dict:
    ...
    return {
        "provider": prov.name,
        # The provider is the transport; the model is what actually wrote the
        # text. Since a gateway lets one provider front any model, a report that
        # names only the provider cannot tell you what it measured.
        "model": getattr(prov, "model", "template"),
        "packs": len(ps),
        "reports": total,
        "grounded": grounded,
...
def report() -> str:
    ...
    out = ["# AI layer evaluation", "",
           f"Generated by `python -m loom.aieval` on the **{s['provider']}** provider, model "
           f"`{s['model']}`, over {s['packs']} evidence packs.", "",
    ...
    out += ["", "## What this does and does not prove", ""]
    if s["provider"] == "template":
        out += ["On the **template** provider a perfect groundedness score is a statement about",
        ...
        out += [f"This is live model output. `{s['model']}` wrote every report scored above, and",
