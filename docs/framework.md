...
8. Control room: `loom/export.py` timelines → `web/build.py` → self-contained replay page; then the live version — `loom/live.py` + `loom/server.py` (FastAPI/WebSocket) + `web/app.html`: real-time plant + twin, clickable 3D stations, fault injection, YAML line editor, persona views, recordings.
9. Topology and calendar: parallel stations (`capacity`), rework loops (`rework_p`/`rework_s`, re-entry as a new thread), breaks and shifts with crew multipliers; supply-aware forecasting (measured inter-arrival instead of assumed takt); leadership ROI view (`economics:`).
10. Persistence and audit (`docs/data_and_audit.md`): `loom/store.py` — events the twin saw, twin events, belief snapshots, audit trail, hashed evidence packs, AI reports with grounding check, replay.
11. Factory I/O adapter (`docs/factoryio.md`): `loom/modbus.py` (dependency-free Modbus/TCP), `loom/factoryio.py` (edges → events → twin, read-only; `ExternalSim` for the control room), `loom/plc_stub.py` (the scene's controller, separate), `loom/fakefactory.py` (Modbus server driven by the plant, for tests and machines without Factory I/O).  ← here
