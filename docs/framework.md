...
9. Factory I/O adapter (`docs/factoryio.md`): `loom/modbus.py` (dependency-free Modbus/TCP), `loom/factoryio.py` (edges → events → twin, read-only; `ExternalSim` for the control room), `loom/plc_stub.py` (the scene's controller, separate), `loom/fakefactory.py` (Modbus server driven by the plant, for tests and machines without Factory I/O).  ← here
